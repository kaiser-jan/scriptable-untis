...
export function addSubjectConfigRow(
	...
	if (subjectOptionKeys.length === 1 && subjectConfig[subjectOptionKeys[0]] === 'teachers') return
	...
	let rowTitle = key
	if ('teacher' in subjectConfig && subjectConfig.teacher) rowTitle += ` with ${subjectConfig.teacher}`
	if (subjectConfig.nameOverride) rowTitle += ` (${subjectConfig.nameOverride})`

	let rowDescription = ''
	if (subjectConfig.longNameOverride) rowDescription += `"${subjectConfig.longNameOverride}"; `
	if (subjectConfig.color) rowDescription += `color: "${subjectConfig.color}"; `
	if (subjectConfig.ignoreInfos) rowDescription += `ignore infos: ${stringifyArray(subjectConfig.ignoreInfos)}; `
	...
	editSubjectButton.onTap = () => openSubjectConfigEditor(key, saveFullConfig, backFunction, subjectConfig)

	// allow editing the subject when clicking anywhere on the row
	subjectRow.onSelect = () => openSubjectConfigEditor(key, saveFullConfig, backFunction, subjectConfig)
